...
def run_full_simulation(magnitude=5.2, lat=18.5204, lon=73.8567, depth_km=10.0, live_readings=None) -> Dict[str, Any]:
    sim = EarthquakeSimulator(epicenter_lat=lat, epicenter_lon=lon, magnitude=magnitude, depth_km=depth_km)
    sensor_data = live_readings if live_readings else sim.generate_sensor_readings()
    return {"trigger": sim.generate_trigger(), "sensor_readings": sensor_data,
