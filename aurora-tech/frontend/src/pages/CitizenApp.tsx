...
export default function CitizenApp() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'ai', content: '⚠️ AI offline. Steps: 1) DROP, COVER, HOLD ON  2) Move to open ground  3) Call 112' }]);
    } finally {
      setSending(false);
    }
  };
  ...
  const submitSOS = async () => {
    try {
      // Use Pune center as default location (in production, use GPS)
      await api.submitSOS({
    ...
    <div style={{ maxWidth: 600, margin: '0 auto' }}>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
          <div className="sim-field">
            ...
            <input type="number" min={1} max={5} value={sosForm.severity}
              onChange={e => setSosForm(p => ({ ...p, severity: +e.target.value }))} />
          </div>
          <div className="sim-field">
            <label>People Count</label>
            <input type="number" min={1} value={sosForm.people}
        ...
        <div style={{ display: 'flex', gap: 16, marginBottom: 12 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: '0.85rem' }}>
            <input type="checkbox" checked={sosForm.medical}
              onChange={e => setSosForm(p => ({ ...p, medical: e.target.checked }))} />
            Needs Medical
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: '0.85rem' }}>
            <input type="checkbox" checked={sosForm.trapped}
              onChange={e => setSosForm(p => ({ ...p, trapped: e.target.checked }))} />
            Trapped
          </label>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <input className="chat-input" placeholder="Describe your situation..."
            value={sosForm.message} onChange={e => setSosForm(p => ({ ...p, message: e.target.value }))} />
      ...
      <div className="card" style={{ marginTop: 24 }}>
        ...
        <div style={{ fontSize: '0.85rem', lineHeight: 2 }}>
          ...
          <div style={{ marginTop: 12, color: 'var(--aurora-red)', fontWeight: 600 }}>
